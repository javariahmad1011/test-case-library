...
Authentication is treated as a business-critical security boundary. The workbooks validate not only successful credential flows but also lockout, verification/reset tokens, account states, password policy, session invalidation, enumeration resistance, accessibility, compatibility, and recovery from interrupted requests.

## Test Assets

| Workbook | Cases | Primary Coverage |
|---|---:|---|
| `Login_Test_Cases.xlsx` | 32 | Credential validation, lockout, sessions, protected routes, security, accessibility, compatibility |
| `Registration_Test_Cases.xlsx` | 32 | Identity creation, uniqueness, verification, consent, password policy, duplicate submission |
| `Forgot_Password_Test_Cases.xlsx` | 27 | Reset requests, token lifecycle, account enumeration, session invalidation, audit |
| `Change_Password_Test_Cases.xlsx` | 27 | Current-password checks, history/complexity, session behavior, security and usability |
